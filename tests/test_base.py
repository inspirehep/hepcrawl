...
def test_date_published(results):
    ...
    print(results['date_published'])
    assert results['date_published']
    assert results['date_published'] == date_published
...
def test_authors(results):
    ...
    assert results['authors']
    assert len(results['authors']) == len(authors)

    # here we are making sure order is kept
    for index, name in enumerate(authors):
        assert results['authors'][index]['full_name'] == name
