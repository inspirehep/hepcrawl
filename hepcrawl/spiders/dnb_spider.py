...
class DNBSpider(XMLFeedSpider):
    ...
    url = ("https://services.dnb.de/oai/accessToken~" + access_token +
    "/repository?verb=ListRecords&metadataPrefix=MARC21-xml&from=2013-12-08"
    "&until=2014-12-10&set=dnb-all:online:dissertations:sg530")

    You should perhaps fetch also DDC 520 theses, not only 530.

    NOTE: you need the access token
    ...
    .. code-block:: console
        ...
        scrapy crawl DNB -a source_file=file://`pwd`/tests/responses/dnb/test_1.xml
    ...
    def get_affiliations(node):
        ...
        affiliations_raw = node.xpath(
            './slim:datafield[@tag="502"]/slim:subfield[@code="a"]/text()'
        ).extract()
        affiliations = []
    ...
    def get_authors(self, node):
        ...
        authors_raw = node.xpath(
            './slim:datafield[@tag="100"]/slim:subfield[@code="a"]/text()'
        ).extract()
    ...
    def get_thesis_supervisors(node):
        ...
        supervisors_raw = node.xpath(
            './slim:datafield[@tag="700"][slim:subfield[@code="e"]'
            '[contains(text(), "Betreuer")]]/slim:subfield[@code="a"]/text()'
        ).extract()
    ...
    def get_urls_in_record(node):
        ...
        urls_in_record = node.xpath(
            './slim:datafield[@tag="856"]/slim:subfield[@code="u"]/text()'
        ).extract()
    ...
    def scrape_for_abstract(self, response):
        ...
        if ('publikationen.ub.uni-frankfurt.de' in domain or
            ...
            page_nr = node.xpath(
                '//tr[./th[contains(text(), "Pagenumber")]]/td/text()'
            ).extract()
            ...
            abstract_raw = node.xpath(
                '//div[@class="simple-item-view-abstract"]/span/text()'
            ).extract()
        elif 'hss.ulb.uni-bonn.de' in domain:
            abstract_raw = node.xpath(
                './/text()[contains(.,"Zusammenfassung") or '
                'contains(., "Abstract")]/ancestor::*[self::tr]/'
                'descendant::*[position() > 1]/text()').extract()
        elif 'kups.ub.uni-koeln.de' in domain:
            abstract_raw = node.xpath(
                '//div[@class="ep_summary_content_main"]/h2/'
    ...
    def build_item(self, response):
        ...
        record.add_xpath(
            'language',
            './slim:datafield[@tag="041"]/slim:subfield[@code="a"]/text()')
